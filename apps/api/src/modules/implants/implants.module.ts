...
export class ImplantsController {
  ...
  getRecord(@Param('id') id: string) {
    return this.prisma.surgicalImplantRecord.findUniqueOrThrow({
      where: { id },
      include: { radiologyAttachments: { include: { mediaAsset: true } } },
    });
  }
  ...
  update(@Param('id') id: string, @Body() dto: UpdateImplantDto) {
    return this.prisma.surgicalImplantRecord.update({ where: { id }, data: dto as never });
  }
  ...
  stageRecords(@Param('stageId') stageId: string) {
    return this.prisma.surgicalImplantRecord.findMany({
      ...
      include: { radiologyAttachments: true },
  ...
  async createForStage(
    ...
    return this.prisma.surgicalImplantRecord.create({
      data: {
        ...
        implantLabel: dto.implantLabel ?? `Имплантат #${dto.implantNumber}`,
        jawScope: dto.jawScope,
        side: dto.side ?? 'UNKNOWN',
  ...
  attach(@Param('id') id: string, @Body() dto: AttachRadiologyDto) {
    return this.prisma.implantRadiologyAttachment.create({
      data: {
        surgicalImplantRecordId: id,
        mediaAssetId: dto.mediaAssetId,
        attachmentType: dto.attachmentType as never,
      },
    });
  }
...
@Module({ controllers: [ImplantsController] })
