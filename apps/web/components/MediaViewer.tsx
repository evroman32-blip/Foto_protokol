...
// Под HTTPS браузер блокирует http-ссылки MinIO, поэтому такой запасной адрес бесполезен
...
export function MediaViewer({ assets, initialIndex, onClose }: MediaViewerProps) {
  ...
  async function handleMediaError() {
    ...
    if (mediaFallback.current === 0 && view.url && src !== view.url && isUsableUrl(view.url)) {
