...
const ACCOUNT_ROLE_OPTIONS = Object.entries(USER_ROLE_LABELS).filter(
  ([value]) => value !== 'MODERATOR' && value !== 'SYSTEM_ADMIN',
);

export default function NewStaffPage() {
  const router = useRouter();
  const { canEditStaff, canAssignAccountRole, loading: userLoading } = useCurrentUser();
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      const member = await staffApi.create({
        ...
        clinicalRoles: roles,
        userRole: canAssignAccountRole ? String(fd.get('userRole') || '') || undefined : undefined,
      });
  ...
  if (!userLoading && !canEditStaff) {
    return (
      <div>
        ...
        <div className="alert-error">Создавать карточку сотрудника могут модератор, исполнительный директор, управляющий клиникой, администратор клиники и главный врач.</div>
      </div>
    );
  }
      ...
      <form onSubmit={handleSubmit} className="card max-w-2xl space-y-4">
        ...
        <div>
          ...
          <p className="mt-1 text-xs text-gray-500">
            Модератор — полные права управления сайтом. Не путать с должностью «Администратор
            клиники».
          </p>
