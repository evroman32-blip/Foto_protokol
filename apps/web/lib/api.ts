...
export interface ProtocolAdminDto {
  id: string;
  code: string;
  name: string;
...
export const adminApi = {
  ...
  protocols: () => request<ProtocolAdminDto[]>('/api/v1/admin/protocols'),
  createProtocol: (data: {
    code: string;
    name: string;
    description?: string;
    ...
    request<ProtocolAdminDto>('/api/v1/admin/protocols', {
      method: 'POST',
      body: data,
    }),
  updateProtocol: (
    ...
    request<ProtocolAdminDto>(`/api/v1/admin/protocols/${id}`, {
      method: 'PATCH',
      body: data,
    }),
  updateProtocolVersion: (
    id: string,
    data: Partial<{ version: string; status: 'DRAFT' | 'PUBLISHED' | 'ARCHIVED' }>,
  ) =>
    request<ProtocolVersionDto>(`/api/v1/admin/protocol-versions/${id}`, {
      method: 'PATCH',
      body: data,
    }),
